...
const AddCurrency = async (req, res) => {
    try {
        ...
        if (!errors.isEmpty()) {
            ...
            if (docs.length > 0) {
                res.status(400).json([
                    { msg: "Currency already added for this store" },
                ]);
            } else {
                ...
                newCurrency
                    ...
                    .catch((err) => {
                        res.status(400).json([
                            { msg: "Currency addition error", error: err },
                        ]);
                    });
            }
        }
        ...
        res.status(400).json([
            { msg: "Currency addition error", error: error.message },
        ]);
    }
};
                ...
                currencies: response.map((element) => {
                    return {
                        ...
                        requests: {
                            DELETE: `${config.get(
                                "prodBaseURL"
                            )}/currency/single-currency/${element._id}`,
                        },
                    };
                }),
            };
